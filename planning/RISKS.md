...
| 401 handler fires during `restoreSession` if stored token is expired — signOut called twice | Low / SEV-5 | signOut is idempotent; double call is harmless, Zustand handles concurrent sets | Accept; monitor if navigation side-effects appear in future auth work |
